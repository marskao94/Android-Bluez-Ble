...
#include "att.h"
#include "eir.h"
#include "utils.h"		//added by Mars
#include "gattrib.h"	//added by Mars


/* nFore_KC_Modify : For our Debug */
#define NF_DEBUG		/**< Enable n4 debugging output */

#if defined(NF_DEBUG)
/** Debug printf macro */
// #define DPRINTF(fmt, ...) do { fprintf(stderr, __FILE__"[%d] %s(): " fmt, __LINE__, __func__,  ## __VA_ARGS__); } while (0)
#define DPRINTF(fmt, ...) do { fprintf(stderr, "%s [%d] : " fmt, __func__, __LINE__, ## __VA_ARGS__); } while (0)
#else /* defined(NF_DEBUG) */
#define DPRINTF(fmt, ...) do { /* NOP */; } while (0)
#endif /* defined(NF_DEBUG) */

#define MAX_PAIR_DEVICES 7
#define REMOVE_PAIR_DEVICES 1 
...
static GSList *adapter_address = NULL;	//added by Mars for recording adapter address
static struct btd_adapter *curr_adapter = NULL;	//added by Mars for recording current adapter


#define NOTIFICATION 1	//added by Mars
#define INDICATION 2	//added by Mars

GAttrib *attrib = NULL;	//added by Mars

float circumference= 0.0;	//added by Mars
//static char wReq = 0;	//added by Mars
...
uint16_t ghandle;
char guuid[5];
uint16_t conn_handle_BQB;


static int no_conn = 0;	//couter for numbers of connection
struct addrs_info {
	char addr[18];
	uint16_t handle;
	struct addrs_info *next;
};
struct addrs_info *devices_head = NULL;
struct addrs_info *devices_curr = NULL;
struct addrs_info *devices_prev = NULL;

static gchar temp_dst[18]; //added by mars
					...
					DBusMessage *msg, void *data)
{
	...
	if (adapter_find_device(adapter, address)){
		g_dbus_emit_signal(conn, adapter->path,
			ADAPTER_INTERFACE, "DeviceExists",	
			DBUS_TYPE_STRING, &address,
			DBUS_TYPE_INVALID);
		DPRINTF("KC_0630 Check btd_error_already_exists()2\n");
		return NULL;
					...
					const char *name, uuid_t uuid, uint8_t channel)
{
	...
	uuid_t root_uuid, l2cap_uuid, rfcomm_uuid, 
			svclass_uuid, ga_svclass_uuid;
	sdp_list_t *svclass_id, *pfseq,
				*svclass, *root, *proto;
	sdp_record_t *record;
	sdp_profile_desc_t profile;

	uint16_t u16 = 0x3D; // Modify 0x31 to 0x3D
	...
	/* nFore_KC_Modify */ 
	if (strstr(name,"nForeTek_HFP")){
		fprintf(stderr, " src/adapter.c/create_rfcomm_record()	[nForeTek_HFP]_#02 Bohan \n");
    ...
    else if(strstr(name,"OBEX PBAP Client")){
        fprintf(stderr, " src/adapter.c/create_rfcomm_record()  [OBEX PBAP Client]_# joseph \n");
        ...
        sdp_set_service_classes(record, svclass_id);   
        sdp_uuid16_create(&profile.uuid, PBAP_PROFILE_ID);
	...
	if (strstr(name,"OBEX PBAP Client")){
        uuid_t obex_uuid;
	...
	if (!strstr(name,"nForeTek_HFP") && !strstr(name,"OBEX PBAP Client")){
		fprintf(stderr, " src/adapter.c/create_rfcomm_record()	[!nForeTek_HFP]_#03 Bohan \n");
    	svclass = sdp_list_append(NULL, &uuid);
    	sdp_set_service_classes(record, svclass);
		}
    else if (strstr(name,"nForeTek_HFP") || strstr(name,"OBEX PBAP Client")){
		fprintf(stderr, " src/adapter.c/create_rfcomm_record()	[SDP_ATTR_SUPPORTED_FEATURES]_#04 Bohan \n");
					...
					DBusMessage *msg, void *data)
{
	...
	if (!record){
		fprintf(stderr, " src/adapter.c/add_rfcomm_service_record()  [!record] \n");
		return g_dbus_create_error(msg,
...
/***added by Mars***/
static DBusMessage *create_gatt_connection(DBusConnection *conn,
					DBusMessage *msg, void *data)
{
	bdaddr_t bdaddr, local_bdaddr;
	struct btd_adapter *adapter = data;
	struct btd_device *device;
	int device_id, err;
	const gchar *address;
	...
	DBG("start to create LE connection\n");
	if (dbus_message_get_args(msg, NULL, DBUS_TYPE_STRING, &address,
							DBUS_TYPE_INVALID) == FALSE)
			return btd_error_invalid_args(msg);
	...
	/*** added by Mars: create le connection ***/
...
ba2str(&bdaddr, srcaddr);
	...
	adapter_get_address(adapter, &local_bdaddr);
	record_adapter_address(adapter, local_bdaddr);

	err = hci_le_create_conn(dd, interval, window, initiator_filter,
			peer_bdaddr_type, bdaddr, own_bdaddr_type, min_interval,
			max_interval, latency, supervision_timeout,
			min_ce_length, max_ce_length, &handle, 35000);

	conn_handle_BQB = handle;//for BQB testing - le start encryption

	devices_curr = devices_head;

	devices_curr = (struct addrs_info*)malloc(sizeof(struct addrs_info));
	if (devices_curr == NULL){
		DBG("cannot get enough memory\n");
	}else{

		devices_curr->next = NULL;

		ba2str(&bdaddr, devices_curr->addr);
		devices_curr->handle = handle;
		no_conn++;
		
		if(devices_head == NULL)
			devices_head = devices_curr;
		else
			devices_prev->next = devices_curr;

		devices_prev = devices_curr;		
	}	
	...
	if (err < 0) {
		...
		char *dst = "90:59:AF:1F:82:AF";
	
		g_dbus_emit_signal(connection, curr_adapter->path,
				ADAPTER_INTERFACE, "LePropertyChanged",
	...
	if (!err)
		return g_dbus_create_error(msg,
				ERROR_INTERFACE ".Failed",
				"Failed to create gatt_connect");

	return dbus_message_new_method_return(msg);
}

void le_reconnect(bdaddr_t paddr)
{
	
	int err, dd, dev_id;
	bdaddr_t bdaddr;
	int16_t interval, latency, max_ce_length, max_interval, min_ce_length;
	uint16_t min_interval, supervision_timeout, window, handle;
	uint8_t initiator_filter, own_bdaddr_type, peer_bdaddr_type;
	...
	char *dst="00:11:22:33:44:55";
	char target[18];
	ba2str(&paddr, target);


	if (1 == get_bdaddr_type(target))
		peer_bdaddr_type = LE_RANDOM_ADDRESS;
	else
		peer_bdaddr_type = LE_PUBLIC_ADDRESS;
	...
	err = hci_le_create_conn(dd, interval, window, initiator_filter,
			peer_bdaddr_type, paddr, own_bdaddr_type, min_interval,
			max_interval, latency, supervision_timeout,
			min_ce_length, max_ce_length, &handle, 35000);

	if (err < 0) {
		perror("re-connect fail");
		char *state = "DISCONNECTED";
...
static uint16_t get_bdaddr_handle(char *peer_addr)
{

	devices_curr = devices_head;
	while(devices_curr != NULL){

		if(strcmp(peer_addr, devices_curr->addr) == 0){
			return devices_curr->handle;
		}
		
		devices_curr = devices_curr->next;		
	}
...
static DBusMessage *remove_gatt_connection(DBusConnection *conn,
					DBusMessage *msg, void *data)
{DPRINTF("\n");
	bdaddr_t bdaddr;
	struct btd_adapter *adapter = data;
	struct btd_device *device;
	int device_id, err, dd;
	const gchar *address;	
	uint16_t handle;
	uint8_t reason;
	char dev_id[5];
	...
	DBG("start to remove connection\n");
	if (dbus_message_get_args(msg, NULL, DBUS_TYPE_STRING, &address,
							DBUS_TYPE_INVALID) == FALSE)
			return btd_error_invalid_args(msg);
...
DPRINTF("address=%s\n", address);	
	...
	address_get_handle(device_id, (long)&bdaddr, &handle);

	reason = HCI_OE_USER_ENDED_CONNECTION;
DPRINTF("handle1=%d\n", handle);

	handle = get_bdaddr_handle(address);
	no_conn--;
DPRINTF("handle2=%d\n", handle);

	err = hci_disconnect(dd, handle, reason, 5000);
	if (err < 0) {
	...
	if (!err)
		return g_dbus_create_error(msg,
				ERROR_INTERFACE ".Failed",
				"Failed to remove gatt_connect");

	return dbus_message_new_method_return(msg);
}

void record_adapter_address(struct btd_adapter *adapter)
{
	adapter_address = g_slist_append(adapter_address, adapter);
}

//when program is finish, this function need to be called
void btd_unregister_adapter(struct btd_adapter *adapter)
{
	adapter_address = g_slist_remove(adapter_address, adapter);
}

static void address_find_adapter(struct btd_adapter *adapter, bdaddr_t *address)
{
	GSList *l;
	char inner_addr[18];
	char ask_addr[18];

	for (l = adapter_address; l; l = l->next){
		adapter = l->data;
		ba2str(&adapter->bdaddr, inner_addr);
		ba2str(address, ask_addr);
		
		if ( strcmp(inner_addr, ask_addr)==0 )
			break;
	}
...
/* nFore_KC.H_Modify : Add events handler to receive notification */
static void events_handler(const uint8_t *pdu, uint16_t len, gpointer user_data)
{

	uint8_t opdu[ATT_MAX_MTU];
	uint16_t handle, i, olen = 0;
	static uint16_t new_handle;	//added by Mars for testing handle value
	char value[50];
	char temp_str[2];
	...
	case ATT_OP_HANDLE_NOTIFY:
		DPRINTF("Notification handle = 0x%04x value: ", handle);
		break;
	case ATT_OP_HANDLE_IND:
		DPRINTF("Indication handle = 0x%04x value: ", handle);
		break;
	default:
		DBG("Invalid opcode\n");
		return;
	...
	for (i = 3; i < len; i++){
		DPRINTF("pdu[%d]=%02x\n", i, pdu[i]);
		sprintf(temp_str, "%02x", pdu[i]);
		if (i == 3 ){
			strcpy (value, temp_str);
		}else if (i > 3 ){
	...
	if (pdu[0] == ATT_OP_HANDLE_NOTIFY){
		...
		g_dbus_emit_signal(connection, curr_adapter->path,
				ADAPTER_INTERFACE, "CharacteristicNotify",
				DBUS_TYPE_UINT16, &new_handle,
				DBUS_TYPE_STRING, &str_value,
				DBUS_TYPE_INVALID);
		return;
	}else if (pdu[0] == ATT_OP_HANDLE_IND){
	
		g_dbus_emit_signal(connection, curr_adapter->path,
				ADAPTER_INTERFACE, "CharIndicate",
				DBUS_TYPE_UINT16, &new_handle,
				DBUS_TYPE_STRING, &str_value,
				DBUS_TYPE_INVALID);
	}else{
...
/* nFore_KC.H_Modify : Register notification/indication handler */
guint gatt_register_events_handler(gpointer user_data)
{
...
void gatt_connect_cb_bond(GIOChannel *io)
{

	attrib = g_attrib_new(io);
	update_attr(attrib);

	DBG("gatt connection complete\n");
	
	g_idle_add(gatt_register_events_handler, attrib);
}	

static void gatt_connect_cb(GIOChannel *io, GError *err, gpointer user_data)
{
	char *state = "CONNECTED";
	char *target_addr = user_data;
	uint16_t conn_handle;
	int gerr = 0;


	conn_handle = get_bdaddr_handle(target_addr);


	if (err) {
		g_printerr("%s\n", err->message);
		DBG("gatt connection fail\n");
		...
		if (1 != nfore_service_discovery(uuid, target_addr, attrib))
			gerr = 1;
	}
	...
	if (gerr){
		...
		//remove connection
		char *state = "DISCONNECTED";
		char *dst = target_addr;
		DBG("notify disconnection\n");
		...
		err = hci_disconnect(dd, conn_handle, reason, 5000);
		if (err < 0) {
			perror("Could not disconnect");
		}
		hci_close_dev(dd);
...
void conn_notify(char *peer_addr)
{
...
void disconn_notify(bdaddr_t bdaddr, uint8_t reason)
{
	...
	conn_handle = get_bdaddr_handle(dst);

	if(reason == 0x08){
		char *state = "LINKLOSS";
		g_dbus_emit_signal(connection, curr_adapter->path,
				ADAPTER_INTERFACE, "LePropertyChanged",
				DBUS_TYPE_STRING, &dst_addr,
				DBUS_TYPE_STRING, &state,
				DBUS_TYPE_INVALID);	
	}else{
		char *state = "DISCONNECTED";
...
void char_read_info(char *value, char *uuid)
{
	int i = 0;
	...
	g_dbus_emit_signal(connection, curr_adapter->path,
				ADAPTER_INTERFACE, "CharReadInfo",
				DBUS_TYPE_STRING, &str_value,
				DBUS_TYPE_STRING, &str_uuid,
				DBUS_TYPE_INVALID);	

	DBG("notify characteristic read\n");

}

static DBusMessage *ble_char_read(DBusConnection *conn,
					DBusMessage *msg, void *data)
{
	bdaddr_t bdaddr, local_bdaddr;
	struct btd_adapter *adapter = data;
	...
	DBG("running characteristic read...\n");
	if (dbus_message_get_args(msg, NULL,
							DBUS_TYPE_STRING, &address,
	...
	err = nfore_char_read(handle, uuid, attrib);
...
void ble_primary_cb(const gchar *start, const gchar *end, const gchar *uuid)
{

		DBG("services have discovered\n");
		g_dbus_emit_signal(connection, curr_adapter->path,
				ADAPTER_INTERFACE, "PrimaryServiceInfo",
				DBUS_TYPE_STRING, &start,
...
void ble_char_cb(const gchar *handle, const gchar *properties, const uint16_t value_handle, const gchar *uuid)
{

		DBG("characteristiccs have discovered\n");
		g_dbus_emit_signal(connection, curr_adapter->path,
				ADAPTER_INTERFACE, "CharacteristicInfo",
				DBUS_TYPE_STRING, &handle,
				DBUS_TYPE_STRING, &properties,
				DBUS_TYPE_UINT16, &value_handle,
				DBUS_TYPE_STRING, &uuid,
				DBUS_TYPE_INVALID);
}

void ble_char_desc_cb(const uint16_t value_handle, const gchar *uuid, const gchar *char_uuid)
{
	g_dbus_emit_signal(connection, curr_adapter->path,
				ADAPTER_INTERFACE, "CharDescriptor",
				DBUS_TYPE_UINT16, &value_handle,
				DBUS_TYPE_STRING, &uuid,
				DBUS_TYPE_STRING, &char_uuid,
				DBUS_TYPE_INVALID);
}

int nfore_gatt_connect(const gchar *src, const gchar *dst)
{
	
	static gchar *opt_sec_level = NULL;
	static int opt_mtu = 48;
	static int opt_psm = 0;
	GIOChannel *chan;

	opt_sec_level = g_strdup("low");

	chan = gatt_connect(src, dst, opt_sec_level, opt_psm, opt_mtu, gatt_connect_cb);
...
static DBusMessage *ble_service_discovery(DBusConnection *conn,
					DBusMessage *msg, void *data)
{
	bdaddr_t bdaddr, local_bdaddr;
	struct btd_adapter *adapter = data;
	...
	char saddr[18];
	ba2str(&adapter->bdaddr, saddr);

	if (1 != isflag3e()){
		
		DBG("ready to gatt_connect\n");
		if (1 != nfore_gatt_connect(saddr, address)){
			error("link layer has connected but l2cap connection is failed");
			return g_dbus_create_error(msg,
				ERROR_INTERFACE ".Failed",
				"Failed to gatt_connect");
		}

	}else{
		//bdaddr_t paddr;
		//str2ba(address, &paddr);
		//le_reconnect(paddr);
		error("connect fail and then connecting again");
	}

	return dbus_message_new_method_return(msg);
}

static DBusMessage *ble_char_desc_discovery(DBusConnection *conn,
					DBusMessage *msg, void *data)
{
	bdaddr_t bdaddr, local_bdaddr;
	struct btd_adapter *adapter = data;
	...
	DBG("Characteristics description are discovering...\n");
	if (dbus_message_get_args(msg, NULL,
							DBUS_TYPE_STRING, &address,
	...
	err = nfore_characteristics_desc(start, end, uuid);

	if (!err)
		return g_dbus_create_error(msg,
				ERROR_INTERFACE ".Failed",
				"Failed to ble_char_desc_discovery");

	return dbus_message_new_method_return(msg);
}

static DBusMessage *ble_char_discovery(DBusConnection *conn,
					DBusMessage *msg, void *data)
{
	bdaddr_t bdaddr, local_bdaddr;
	struct btd_adapter *adapter = data;
	...
	DBG("Characteristics are discovering...\n");
	if (dbus_message_get_args(msg, NULL,
							DBUS_TYPE_STRING, &address,
							DBUS_TYPE_UINT16, &start,
							DBUS_TYPE_UINT16, &end,
	...
	err = nfore_characteristics(start, end, uuid, attrib);

	if (!err)
		return g_dbus_create_error(msg,
				ERROR_INTERFACE ".Failed",
				"Failed to ble_char_discovery");

	return dbus_message_new_method_return(msg);
}


/* nFore_KC_Modify : Add char_write req cb for Notification */
static void char_write_req_cb(guint8 status, const guint8 *pdu, guint16 plen,
							gpointer user_data)
{
	char *uuid = user_data;
	if (status != 0) {
		DBG("Characteristic Write Request failed: "
	...
	uint16_t handle = ghandle;
	//char uuid[5];
	//strcpy(uuid, guuid);
	
	g_dbus_emit_signal(connection, curr_adapter->path,
				ADAPTER_INTERFACE, "CharWrite",			
				DBUS_TYPE_STRING, &uuid,				
				DBUS_TYPE_UINT16, &handle,			
				DBUS_TYPE_INVALID); 
...
/* nFore_KC_Modify : Add char_write req for Notification */
static DBusMessage *ble_char_indicate(DBusConnection *conn,
					DBusMessage *msg, void *data)
{
	uint16_t *value=NULL;
	...
	size_t len;
	int err=1;
	uint8_t *nf_value = NULL;
	char curr_value[4] ;
		
	DBG("Characteristic is indicating...\n");
	if (dbus_message_get_args(msg, NULL,
							DBUS_TYPE_STRING, &address,
							DBUS_TYPE_UINT16, &handle,							
	...
	//DPRINTF("Handle =  0x%04x\n", handle);	
	if (handle <= 0) {
		g_printerr("A valid handle is required\n");
		err = 0;
	}
	
	sprintf(curr_value, "%04x", value);
	...
	strcpy(temp_dst, address);
	len = gatt_attr_data_from_string(curr_value, &nf_value);
	...
	if (err!=0)
		err = gatt_write_char(attrib, handle, nf_value, len, char_write_req_cb,
									NULL);
	if (!err)
			return g_dbus_create_error(msg,
					ERROR_INTERFACE ".Failed",
					"Failed to ble_char_read");
...
/* nFore_KC_Modify : Add char_write req for Notification */
static DBusMessage *ble_char_write_req(DBusConnection *conn,
					DBusMessage *msg, void *data)
{
	...
	/*if(!strcmp( curr_value, "0100" )){
		DBG("notify\n");
		wReq = NOTIFICATION;
	}else if (!strcmp( curr_value, "0200" )){
		DBG("indicate\n");
		wReq = INDICATION;
	}*/
	...
	len = gatt_attr_data_from_string(value, &nf_value);
...
static void char_write_cb(gpointer user_data)
{
	gchar *str_value = user_data;
...
static DBusMessage *ble_char_write(DBusConnection *conn,
					DBusMessage *msg, void *data)
{
	...
	DBG("running characteristic write\n");
	if (dbus_message_get_args(msg, NULL,
					DBUS_TYPE_STRING, &address,
					DBUS_TYPE_UINT16, &handle,							
					DBUS_TYPE_STRING, &str_value,
	...
	if (err!=0)
		gatt_write_cmd(attrib, handle, value, len, char_write_cb, str_value);

	if (!err)
		return g_dbus_create_error(msg,
				ERROR_INTERFACE ".Failed",
				"Failed to ble_char_write");
		
	return dbus_message_new_method_return(msg);
	
}

static void le_start_encryption()
{

		/*LE start encryption*/
		struct hci_request rq;	
		le_start_encryption_cp	start_encrypt_cp;
		...
		memset(&start_encrypt_cp, 0, sizeof(start_encrypt_cp));
		start_encrypt_cp.handle = conn_handle_BQB;
		start_encrypt_cp.random = 0;
		start_encrypt_cp.diversifier = 0;
		start_encrypt_cp.key[16] = 0;

		memset(&rq, 0, sizeof(rq));
		rq.ogf = OGF_LE_CTL;
		rq.ocf = OCF_LE_START_ENCRYPTION;	
		rq.event   = EVT_ENCRYPT_CHANGE;
		rq.cparam = &start_encrypt_cp;
		rq.clen = LE_START_ENCRYPTION_CP_SIZE;
		rq.rparam = &rp;
		rq.rlen = EVT_ENCRYPT_CHANGE_SIZE;
		DBG("ready to le start encryption\n");
		ret = hci_send_req(dd, &rq, 1000);
		if (ret < 0)
			perror("Could not send LE_START_ENCRYPTION");

}
		


void encryption_change_notify(uint8_t status, uint16_t handle, uint8_t encrypt)
{

	if(conn_handle_BQB == handle){
		
		g_dbus_emit_signal(connection, curr_adapter->path,
				ADAPTER_INTERFACE, "EncryptionChanged",
...
static GDBusMethodTable adapter_methods[] = {
	...
	//added by Mars	for	BLE		
	{ "BleChard",		"sqqs",		"",			ble_char_discovery },
	{ "SerDis",			"ss",		"",			ble_service_discovery },	
	{ "BleDisconnect", 	"s",		"", 		remove_gatt_connection},
	{ "BleConnect", 	"s",		"", 		create_gatt_connection },
	{ "ClientCharDesc",	"sqqs",		"",			ble_char_desc_discovery },
	{ "CharRead",		"sqs",			"",			ble_char_read },
	{ "CharacteristicWriteReq",  "sqss", 	"",		ble_char_write_req},		
	{ "CharacteristicIndicate",  "sqq", 	"",		ble_char_indicate},			
	{ "characteristicsWrite",    "sqss",		"",		ble_char_write},
	{ }
};
...
static GDBusSignalTable adapter_signals[] = {
	...
	{ "DeviceFound",		"sa{sv}"	},
	{ "DeviceDisappeared",		"s"		},
	{ "DeviceExists",		"s"		},	//added by KC
	//added by Mars for BLE
	{ "LePropertyChanged",		"ss"	},		
	{ "PrimaryServiceInfo",		"sss"	},		
	{ "CharacteristicInfo",		"ssqs"  },		
	{ "CharDescriptor",			"qss"  	},		
	{ "CharReadInfo",			"ss"	},		
	{ "CharacteristicNotify",	"qs"	},		
	{ "CharIndicate", "qs"	},		
	{ "CharWrite", "sq"		},
	{ "EncryptionChanged", "yqy"	},
	{ }
};
						...
						struct remote_dev_info *dev)
{
	...
	if (dev->le) {
		DBG("adapter_emit_device_found - device_le\n");	//added by Mars
...
/* 
  * nFore_KC_Modify : for limit the max number of paired device 
  * According to the official answer from Android, the proper max value is 7,
  * however, by our real IOP result, 10 would still be acceptable
  */
gboolean adapter_check_and_remove_paired_devices(void *data)
{
	...
	// First check the current number of paired devices
	iPairCount = g_slist_length(adapter->devices);

	if (iPairCount > MAX_PAIR_DEVICES){

		DPRINTF("Max pair limit is reached, Unpair devices... \n");

		int i;

		// Copy the original paired list
		pgslRef = g_slist_copy(adapter->devices);
		
		for (i=0; i<REMOVE_PAIR_DEVICES; (pgslRef = pgslRef->next)){

			device = pgslRef->data;
			
			if (device_is_temporary(device) || device_is_busy(device)){
				DPRINTF("Try next devices, device is busy, current index = %d\n", i);
				continue;
			}
			
			device_set_temporary(device, TRUE);

			// However, how could we make sure that the Java layer HFP is connected or not ???
			if (device_is_connected(device)){
				DPRINTF("Try next devices, device is connected, current index = %d\n", i);
				device_set_temporary(device, FALSE);
				continue;			
			}
		...
		DPRINTF("Unpair devices has completed \n");
